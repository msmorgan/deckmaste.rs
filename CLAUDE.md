...
- **Every completed ticket carries a `## Landing record`** written by the claimant before integrate: numbers before/after (coverage, construction count, lock state, the permitted licensing-checker total emitted by `coverage`, and the selection census — unique / specificity-resolved — so a shift in which analysis wins is quantified, not just mentioned) **stamped with the change id and lock `covered` count of the tree they were measured on** (an integrate rebase can move every corpus figure while both lock hashes still pass), the positive gate artifacts, a **performance advisory** (coverage-command wall time against the 16.26s quiet-host ceiling and the per-byte thread-CPU telemetry line — written as an integer in nanoseconds per byte, e.g. `103,762 ns/B`, never `103.762 µs/B`, which the cite checker reads as a rule number — so a parse-time regression is visible at review rather than discovered by the next perf round), **every identity newly covered by the landing listed with its selected analysis** (including identities exposed only by a refresh — a negative oracle or a wrong analysis that starts parsing is a defect and a STOP, never a coverage gain, and blessing it into the lock is the exact shape the 2026-09-03 require-through landing shipped), a **Deviations and additions** list (every construction or test added or deleted beyond the ticket's letter, with justification), and any STOP taken with its resolution. A landing without one is a review finding; a ticket-vs-ruling contradiction resolved without a STOP is a HIGH finding — **even when the resolution is disclosed and well-measured**: stop on a contradiction with a recorded ruling even if you can resolve it, because disclosure is not authority. A guard that names a lexeme, construction, verb, noun, preposition, or card identity (`head != CoreVerbIdentity::X`, `require x is <Construction>`, a comment naming a card) is a STOP-and-report, never shipped — the permitted guard reads a declared feature. Ledger residue routes to live tickets at integrate. A coverage-lock retirement (`--bless --retire`) is a coordinator ruling: it is refused unless the manifest is untracked and the claimed ticket's `## Landing record` contains a line `retirement/re-coverage obligation: <every retired identity>` (accepted labels: `re-coverage obligation:`, `retirement/re-coverage obligation:`, `re-coverage/retirement obligation:`, `re-coverage or retirement obligation:`) — one line naming all retired identities.
- **Lock ratchet SUSPENDED for the generalization refactors (ruling 2026-09-04).** The add-only/monotonic constraint on `english-v2-coverage.lock` and the retirement-manifest/obligation-line ceremony are off until the remedy tickets from the 2026-09-04 fallout audit have landed (the coordinator lifts this line then). While suspended: a landing may lower or raise the lock freely and blesses it to whatever the tree covers; the `## Landing record` still lists every identity that stopped being covered with its cause (wrong analysis retired / re-coverage owed to a named live ticket / regression) and every identity newly covered with its selected analysis; an UNEXPLAINED loss is the defect, a decrease is not. The invariants stay as gates: zero unresolved ties, roundtrip byte-exactness, ownership and traversal failures at zero, no negative oracle or wrong analysis blessed (still a STOP), no word-naming guard, `cargo test --workspace` for emit changes. Metrics designed for the retired corpus-driven plan (exact pins on inventory counts, overlap ceilings, the perf ceiling, "coverage up" as success) are reported as provenance, never fitted to. Why: the 2026-09-04 rounds show every intentional transient of a restructuring (deleting fused constructions, narrowing a wrong analysis) becoming either a contortion to hold the number or a STOP round-trip.
