...
## Feature workflow (`scripts/workflow`, run from `default` — except `refresh`/`repair`/`converge`/`resolve`, which run from the feature workspace)
...
- **Taking on an extra todo mid-feature** ("oh, we're also fixing this now"): `scripts/workflow claim TODO... --into NAME` folds one or more TODOs into the **existing** workspace NAME's claim instead of spinning up a new workspace — NAME then owns those tickets too. Run it from `default` like any claim. It amends NAME's claim commit, so `../NAME` goes **stale** (rewritten parent) — that's routine; in `../NAME` run `scripts/jj workspace update-stale` (commit first) before the next commit. `integrate NAME` finishes **every** todo the claim owns and `abandon NAME` reverts them all.
- `refresh` rebases a feature's stack onto the current trunk tip (`default@-`), **detaching it from its claim** (the claim stays in default's line; `integrate` re-joins it). Run it **from the feature workspace with no argument** (the common case — an agent's in-place "get current" call, lock-free so it never waits on the workflow lock), or `refresh NAME` from `default`. **Always `refresh` before any review step** (e.g. before `/code-review`, `requesting-code-review`, or handing work to a reviewer) so the diff is against current trunk, not stale history. `refresh --all` refreshes every non-default workspace in turn (prep for a push), stopping at the first that conflicts or fails — **human-only: never run `refresh --all` (this includes Claude and subagents). It rebases every workspace's stack + un-stales it at once, which races concurrent sessions into working-copy divergence. A Claude touches only its own feature, via `refresh` / `integrate NAME`.** `integrate NAME` (from `default`) refreshes, **re-joins the claim to the now-current feature**, folds the feature into default@, moves the claim's ticket(s) `wip/→done/` (all of them, for a multi-todo `--into` claim), and archives the workspace to `../.integrated/`; `abandon NAME` discards the feature + claim (reverting every ticket it owns to triage), archiving to `../.abandoned/`. `scripts/workflow converge` (run from the feature workspace) heals a working-copy divergence — see Version control.
- **Conflicts/divergence land in the feature workspace, not on trunk** — refresh/integrate exit 2 ("resolve it in ../NAME, then re-run") and never roll back. Recover *inside* `../NAME` with the one-stop **`scripts/workflow repair`** = `update-stale` + `converge` (heals a working-copy divergence) + `resolve` (walks refresh/integrate conflicts oldest-first). **Exit 0** = clean (re-integrate); **exit 1** = you're now on a conflicted commit — remove its markers (the files `jj st` lists) and re-run `repair`; **exit 2** = needs a human (the two divergent halves hold genuinely different work, or a step broke and was rolled back). `converge`/`resolve` are callable directly too. When a conflict is *semantic* (e.g. another feature moved a ticket to `done/` you have elsewhere), reconcile the meaning — move/mint the right tickets — not just the markers. Recover there, not from `default`: a `jj` pinned to `default` (e.g. a sourced workflow alias) inlines default@ onto the feature instead.
