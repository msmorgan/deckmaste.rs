...
- `refresh NAME` reorders the claim to just under `default@` (feature current with trunk); `refresh --all` does that for every non-default workspace in turn (prep for a push), stopping at the first that conflicts or fails — **human-only: never run `refresh --all` (this includes Claude and subagents). It rewrites every workspace's claim at once, which races concurrent sessions into working-copy divergence (a raced `integrate` then folds a *stale* half into default@). A Claude touches only its own feature, via `refresh NAME` / `integrate NAME`.** `integrate NAME` refreshes, folds the feature into default@, moves the claim's ticket(s) `wip/→done/` (all of them, for a multi-todo `--into` claim), and archives the workspace to `../.integrated/`; `abandon NAME` discards the feature + claim (reverting every ticket it owns to triage), archiving to `../.abandoned/`. `scripts/workflow converge` (run from the feature workspace) heals a working-copy divergence — see Version control.
