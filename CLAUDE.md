...
## Current English grammar contract

For English implementation, planning, review or corpus accounting, read
[`docs/decisions/english-lexical-analysis.md`](docs/decisions/english-lexical-analysis.md).
It supersedes the old zero-tie, destructive-selection and span-storage
prescriptions, including conflicting wording in the landing contract above.
...
- **Corpus work iterates on a subset, verifies on the whole.** A full `english_v2` corpus pass costs ~100 s; never use it as an edit loop (one round burned 82). Build the affected subset with jq — by surface (`jq --arg re '<regex>' '.data |= with_entries(select(any(.value[]; (.text // "") | test($re))))' data/mtgjson/AtomicCards.json`), by card names whose parent-tip selected path contains a touched construction, plus witnesses and negatives — pass it to every corpus command via `--data <subset.json>` while developing, and run the full set (coverage `--check`/`--bless`, ambiguity, roundtrip, the per-unit diff) exactly once on the refreshed tree at the end. A full pass before that needs a stated reason in the landing record.
