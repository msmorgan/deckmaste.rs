...
set -l deps_ok
for program in curl jq rg sqlite3
...
set -g migrations_dir $scripts_dir/migrations
set -g lib_dir $scripts_dir/lib

set -p fish_function_path $lib_dir/functions/{fetch,migration}

alias _jq "jq -crM -L$lib_dir/jq -L$data_dir"