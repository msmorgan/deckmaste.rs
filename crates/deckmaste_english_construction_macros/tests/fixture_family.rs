//! The sealed own-mode fixture family, end to end through constructicon!.
//!
...
//! `deckmaste_english_construction_compiler`'s `tests/golden_real.rs`,
//! parsed from byte-identical DSL text in `parse.rs`'s `fixture_dsl()`),
//! compiled through the real `constructicon!` proc macro and exercised at
...
use deckmaste_english_features::Comma;
use deckmaste_english_features::Conjunction;

deckmaste_english_construction_macros::constructicon! {
...
fn declaration_data_traces_to_the_one_declaration() {
    use deckmaste_english_construction_compiler::runtime::AtomData;
