...
mod tests {
    ...
    const PLAN09_PART_IV: &str = include_str!("english_v2/plan09_part_iv_witnesses.tsv");
    ...
    const PLAN09_TASK10D_EXPANSION: &str = include_str!("english_v2/plan09_task10d_expansion.tsv");
    ...
    fn parse_plan09_task9_expansion_inventory() -> Vec<(String, String)> {
        PLAN09_PART_IV
            .lines()
            .filter(|line| line.starts_with("generated\t"))
            .map(|line| {
                let fields = line.split('\t').collect::<Vec<_>>();
                let ["generated", item, origins_digest] = fields.as_slice() else {
                    panic!("Task 9 generated row has three exact fields: {fields:?}");
                };
                (
                    serde_json::from_str(item).expect("Task 9 generated item key is JSON"),
                    (*origins_digest).to_owned(),
                )
            })
            .collect()
    ...
    #[test]
    #[ignore = "explicit Task 9 fixture refresh expands the complete production declaration"]
    fn refresh_plan09_task9_expansion_inventory() {
        let expansion = expansion_from_source(PRODUCTION_SOURCE)
            .expect("production declaration expands from the sealed semantic plan");
        let inventory = expansion_inventory(&expansion);
        let mut rendered = PLAN09_PART_IV
            .lines()
            .take_while(|line| !line.starts_with("# expansion_schema="))
            .collect::<Vec<_>>()
            .join("\n");
        writeln!(&mut rendered).expect("Task 9 fixture section terminates");
        writeln!(&mut rendered, "# expansion_schema=1").expect("expansion schema renders");
        writeln!(&mut rendered, "# generated_item_count={}", inventory.len())
            .expect("expansion count renders");
        writeln!(
            &mut rendered,
            "# expansion_columns=kind\titem_json\torigins_sha256"
        )
        .expect("expansion columns render");
        for (item, origins) in inventory {
            writeln!(
                &mut rendered,
                "generated\t{}\t{}",
                serde_json::to_string(&item).expect("generated item encodes"),
                sha256_hex(origins.join("\0").as_bytes()),
            )
            .expect("generated row renders");
        }
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("src/english_v2/plan09_part_iv_witnesses.tsv");
        fs::write(path, rendered).expect("Task 9 expansion fixture writes");
    }

    #[test]
    #[ignore = "explicit Task 10 fixture refresh expands the complete production declaration"]
    fn refresh_plan09_task10_expansion_inventory() {
        let expansion = expansion_from_source(PRODUCTION_SOURCE)
            .expect("production declaration expands from the sealed semantic plan");
        let rendered = render_plan09_expansion_inventory(&expansion_inventory(&expansion))
            .expect("Task 10 generated expansion inventory renders");
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("src/english_v2/plan09_task10_expansion.tsv");
        fs::write(path, rendered).expect("Task 10 expansion fixture writes");
    }

    #[test]
    #[ignore = "explicit Task 10A fixture refresh expands the complete production declaration"]
    fn refresh_plan09_task10a_expansion_inventory() {
        let expansion = expansion_from_source(PRODUCTION_SOURCE)
            .expect("production declaration expands from the sealed semantic plan");
        let rendered = render_plan09_expansion_inventory(&expansion_inventory(&expansion))
            .expect("Task 10A generated expansion inventory renders");
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("src/english_v2/plan09_task10a_expansion.tsv");
        fs::write(path, rendered).expect("Task 10A expansion fixture writes");
    }

    #[test]
    #[ignore = "explicit Task 10B fixture refresh expands the complete production declaration"]
    fn refresh_plan09_task10b_expansion_inventory() {
        let expansion = expansion_from_source(PRODUCTION_SOURCE)
            .expect("production declaration expands from the sealed semantic plan");
        let rendered = render_plan09_expansion_inventory(&expansion_inventory(&expansion))
            .expect("Task 10B generated expansion inventory renders");
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("src/english_v2/plan09_task10b_expansion.tsv");
        fs::write(path, rendered).expect("Task 10B expansion fixture writes");
    }
    ...
    #[test]
    #[ignore = "explicit Task 10D fixture refresh expands the complete production declaration"]
    fn refresh_plan09_task10d_expansion_inventory() {
        let expansion = expansion_from_source(PRODUCTION_SOURCE)
            .expect("production declaration expands from the sealed semantic plan");
        let rendered = render_plan09_expansion_inventory(&expansion_inventory(&expansion))
            .expect("Task 10D generated expansion inventory renders");
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("src/english_v2/plan09_task10d_expansion.tsv");
        fs::write(path, rendered).expect("Task 10D expansion fixture writes");
    }
    ...
    fn production_expansion_prints_each_literal_item_key_once_with_every_origin() {
        ...
        let task9 = parse_plan09_task9_expansion_inventory();
        ...
        let task9_origin_names = BTreeSet::from([
            ...
            "construction predicative_scalar",
            "construction library_object",
            "construction power_toughness_modifier",
            "construction control_of_reference",
            ...
            "construction declared_for_object_predicate",
            "construction quoted_ability",
            "construction quoted_ability_predicate",
            "construction quote_terminated_statement",
            "construction get_power_toughness",
            "construction have_base_power_toughness",
        ]);
        let task10_origin_names = BTreeSet::from([
            "vocab FlexibleManaKind",
            "vocab StepModifier",
            "vocab MassCommonNoun",
            "lexeme PreventionParticipleLexeme",
            "abstract sum CoordinatedPredicate",
            "abstract sum CoordinatedClause",
            "construction preposed_as",
            "construction blocked_except_by_status",
            "construction all_colors",
            "construction transitive_requirement_predicate",
            "construction step_modifier",
            "construction from_anywhere",
            "construction mass_common_noun_reference",
            "construction damage_prevention_relative",
            "construction prevent_damage",
            "construction flexible_mana",
            "construction enter_with_counters",
        ]);
        let negative_magnitude_origins =
            BTreeSet::from(["construction negative_power_toughness_magnitude"]);
        let task10b_origin_names = BTreeSet::from([
            ...
            "construction object_distribution_recipient",
            "construction bare_target_distribution_recipient",
            "construction bounded_target_distribution_recipient",
            "construction chosen_damage_distribution",
            "construction even_damage_distribution",
            "construction counter_among_source",
            "construction deal_distributed_damage",
            "construction twice_variable_amount",
            "construction variable_plus_amount",
        ]);
        let task10b_modified_origin_names = BTreeSet::from([
            "construction positive_power_toughness_counter",
            ...
            let retained_origins = live_origins
                ...
                .filter(|origin| {
                    !task8_origin_names.contains(origin.as_str())
                        && !task9_origin_names.contains(origin.as_str())
                        ...
                        && !task10d_origin_names.contains(origin.as_str())
                })
                .cloned()
                .collect::<Vec<_>>();
            assert_eq!(
        ...
        for (item, origins_digest) in task8_new_rows {
            ...
            assert_eq!(
                sha256_hex(retained_origins.join("\0").as_bytes()),
                *origins_digest,
                "Task 9 retains exact Task 8 origins for {item:?}",
            );
        }
        ...
        let fixture_by_item = fixture.iter().cloned().collect::<BTreeMap<_, _>>();
        let task9_new_rows = task9
            .iter()
            .filter(|(item, _)| !fixture_by_item.contains_key(item))
            .collect::<Vec<_>>();
        ...
        for (item, origins_digest) in &task9 {
            ...
            let retained_origins = live_origins
                ...
                .filter(|origin| !task10d_origin_names.contains(origin.as_str()))
                .cloned()
                .collect::<Vec<_>>();
            assert_eq!(
                sha256_hex(retained_origins.join("\0").as_bytes()),
                *origins_digest,
                "Task 10 retains exact Task 9 origins for {item:?}",
            );
        }
        for task9_origin in &task9_origin_names {
            assert!(
                live.iter()
                    .any(|(_, origins)| origins.iter().any(|origin| origin == task9_origin)),
                "Task 9 origin is live: {task9_origin:?}",
            );
        }
        let task9_by_item = task9.iter().cloned().collect::<BTreeMap<_, _>>();
        let task10_new_rows = task10
            .iter()
            .filter(|(item, _)| !task9_by_item.contains_key(item))
            .collect::<Vec<_>>();
        assert!(!task10_new_rows.is_empty());
        assert_eq!(task10.len(), task9.len() + task10_new_rows.len());
        for task10_origin in &task10_origin_names {
            assert!(
                live.iter()
                    .any(|(_, origins)| origins.iter().any(|origin| origin == task10_origin)),
                "Task 10 origin is live: {task10_origin:?}",
            );
        }
        let task10_by_item = task10.iter().cloned().collect::<BTreeMap<_, _>>();
        let negative_magnitude_new_rows = task10a
            .iter()
            .filter(|(item, _)| !task10_by_item.contains_key(item))
            .collect::<Vec<_>>();
        assert!(!negative_magnitude_new_rows.is_empty());
        assert_eq!(
            task10a.len(),
        ...
        for (item, origins_digest) in &task10 {
            ...
            assert_eq!(
                sha256_hex(retained_origins.join("\0").as_bytes()),
                *origins_digest,
                "Task 10A retains exact Task 10 origins for {item:?}",
            );
        }
        for task10a_origin in &negative_magnitude_origins {
            assert!(
                live.iter()
                    .any(|(_, origins)| origins.iter().any(|origin| origin == task10a_origin)),
                "Task 10A origin is live: {task10a_origin:?}",
            );
        }
        for (item, _) in negative_magnitude_new_rows {
            let origins = live_origins_by_item
                .get(item)
                .unwrap_or_else(|| panic!("Task 10A new item is live: {item:?}"));
            assert_eq!(
                origins,
                &["construction negative_power_toughness_magnitude".to_owned()],
                "Task 10A new item has exactly the authenticated construction origin: {item:?}",
            );
        }
        let task10a_by_item = task10a.iter().cloned().collect::<BTreeMap<_, _>>();
        let task10b_new_rows = task10b
            .iter()
        ...
        for (item, origins_digest) in &task10a {
            ...
            assert_eq!(
                sha256_hex(retained_origins.join("\0").as_bytes()),
                *origins_digest,
                "Task 10B retains exact Task 10A origins for {item:?}",
            );
        }
        let allowed_task10b_origins = task10b_origin_names
            .union(&task10b_modified_origin_names)
            .copied()
            .collect::<BTreeSet<_>>();
        for (item, _) in task10b_new_rows {
            let origins = live_origins_by_item
                .get(item)
                .unwrap_or_else(|| panic!("Task 10B new item is live: {item:?}"));
            assert!(
                origins
                    .iter()
                    .filter(|origin| !task10c_origin_names.contains(origin.as_str()))
        ...
        for origin in &allowed_task10b_origins {
            assert!(
                live.iter()
        ...
        for (item, origins_digest) in &task10c {
            ...
            assert_eq!(
                sha256_hex(retained_origins.join("\0").as_bytes()),
                *origins_digest,
                "Task 10D retains exact Task 10C origins for {item:?}",
            );
        }
        for (item, _) in task10d_new_rows {
            let origins = live_origins_by_item
                .get(item)
        ...
        for origin in &task10d_origin_names {
            assert!(
                live.iter()
                    .any(|(_, origins)| origins.iter().any(|live_origin| live_origin == origin))
            );
