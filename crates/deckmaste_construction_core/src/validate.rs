    ...
    fn full_golden_shaped_grammar_parses_and_validates_all_output_identities() {
        ...
        assert_eq!(
            ...
            [
                ...
                ("with_where", "WithWhere", "WithWhere"),
                ("event", "Event", "EventClause"),
                ("where", "Where", "WhereClause"),
                ("pronoun", "Pronoun", "PronounNp"),
                ("common", "Common", "Common"),
                ("demonstrative", "Demonstrative", "DemonstrativeNp"),
                ("target", "Target", "TargetNp"),
                ("self_reference", "SelfReference", "SelfReferenceNp"),
                ("count", "Count", "CountNp"),
                ("destroy", "Destroy", "Destroy"),
                ("connive", "Connive", "Connive"),
                ("deal_damage", "DealDamage", "DealDamage"),
                ("gain_life", "GainLife", "GainLife"),
                ("number", "Number", "NumberAmount"),
                ("variable", "Variable", "VariableAmount"),
            ]
        );
        ...
        let refinements = validated
            ...
            .collect::<Vec<_>>();
        assert_eq!(refinements, ["Event", "Where", "You"]);
        ...
        let terminal = |name| {
            validated
                ...
                .unwrap_or_else(|| panic!("terminal `{name}` is inventoried"))
        };
        assert!(terminal("VerbLexeme").supports_verb_atom());
        assert!(!terminal("NounLexeme").supports_verb_atom());
        assert!(terminal("VerbLexeme").has_traversal());
        assert!(terminal("NounLexeme").has_traversal());
        assert!(terminal("Noun").supports_noun_atom());
        assert!(!terminal("Noun").supports_lex_atom());
        assert!(terminal("SignedNumber").supports_lex_atom());
        assert!(!terminal("SignedNumber").supports_noun_atom());
        assert!(terminal("SelfReferenceSpelling").supports_identity_atom());
        assert!(!terminal("SelfReferenceSpelling").supports_lex_atom());

        let self_reference = validated
            .contributions()
            .constructions()
            .iter()
            .find(|construction| construction.construction_id() == "self_reference")
            .expect("self-reference construction is inventoried");
        assert_eq!(self_reference.atoms().len(), 1);
        assert_eq!(
            self_reference.atoms()[0].terminal(),
            Some("SelfReferenceSpelling")
        );
    }
}
