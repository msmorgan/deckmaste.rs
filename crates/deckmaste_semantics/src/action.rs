...
/// in, [CR#110.5,614.12]).
///
/// How much of that is ENFORCED, exactly: only [`Attacking`](Self::Attacking)
/// has an Idris carrier (`Move`'s `enteringAttacking`), and the mirror's
/// `EnteringOk` proof makes it unrepresentable off the battlefield. The other
/// riders have no Idris counterpart at all — the emitter gaps on them (gaps
/// are reported as coverage, never failures), so for those the rule is
/// currently UNENFORCED, by the mirror or anything else. Closing that needs
/// the riders mirrored, which is `idris-mirror-enum-gaps` territory.
#[derive(Debug, Clone, PartialEq, Eq, Hash, SupportsMacros)]
pub enum EnterRider {
...
/// `RemoveCounters`, `Reveal`, `RemoveDamage`). No slot defaults
/// (`#[macro_ron(default)]`/serde-default) on a role field (Law 2) — a bare
/// `Sacrifice(This)` no longer exists; the agent is always spelled
/// (`Sacrifice(You, This)`).
///
/// Both serde impls are generated by `#[derive(SupportsMacros)]`:
/// [`Expanded`](Action::Expanded) writes a remembered macro invocation back.
#[derive(Debug, Clone, PartialEq, Eq, Hash, SupportsMacros)]
pub enum Action {
    ...
    /// (see [`EnterRider`] for which of them the mirror actually proves).
    ///
    ...
    /// `SupportsMacros` tuple-default layout (mirrors
    ...
    Pay(crate::Cost),
    /// A remembered `Action` macro invocation.
    #[macro_ron(expanded)]
    Expanded(Expansion<Action>),
}
...
mod tests {
    ...
    fn is_cost_eligible_covers_self_directed_verbs() {
        assert!(Action::Sacrifice(Reference::You, Reference::This).is_cost_eligible());
        assert!(
            Action::Move(
                Reference::This,
        ...
        assert!(Action::Tap(Reference::This).is_cost_eligible());
        assert!(Action::Untap(Reference::This).is_cost_eligible());
        // The whole `ChangeLife` family is eligible ([CR#119.7] — Invigorate,
        // Skyshroud Cutter print gain-life costs), not just the losing
        // direction.
        assert!(
            Action::ChangeLife(Reference::You, LifeOp::Down(Count::Literal(1))).is_cost_eligible()
        );
        assert!(
            Action::ChangeLife(Reference::You, LifeOp::Up(Count::Literal(1))).is_cost_eligible()
        );
    ...
    /// A player verb's agent slot round-trips through plain `ron` (no
    /// `MacroSet` needed — Law 2 means there is no default to apply).
    #[test]
    fn player_verb_agent_is_spelled_not_defaulted() {
        // Law 2: no read-time default — the agent is always spelled.
        assert_eq!(
            read("Sacrifice(You, This)"),
            Action::Sacrifice(Reference::You, Reference::This),
        );
        // `Tap`/`Untap` are agent-silent ([CR#701.26a..701.26b]) — unchanged,
        // one slot, no agent to spell.
        assert_eq!(read("Tap(This)"), Action::Tap(Reference::This));
    }

    /// A bare, agent-less player verb no longer parses ([CR#608.2] — `By`'s
    /// implicit-you default is gone; the agent is always spelled). An arity
    /// mismatch is a plain parse error, not a fallback.
    #[test]
    fn bare_agentless_player_verb_rejected() {
        assert!(
            crate::ron::options()
                .from_str::<Action>("Sacrifice(This)")
                .is_err()
        );
    }
    ...
    fn explicit_agent_reads_natively() {
        assert_eq!(
            read("Sacrifice(EventActor, This)"),
            Action::Sacrifice(Reference::EventActor, Reference::This),
        );
    }
    ...
    /// `GetDesignation(recipient, name)` round-trips; the recipient is
    /// spelled, never defaulted. The designation name is a quoted Ident
    /// ([CR#702.131c]).
    #[test]
    fn get_designation_round_trips() {
        let v = Action::GetDesignation(Reference::You, "CitysBlessing".into());
        let written = write(&v);
        assert_eq!(read(&written), v);
        assert_eq!(
            read(r#"GetDesignation(You, "CitysBlessing")"#),
            v,
            "the recipient reads in the leading slot"
        );
    }

    /// [CR#114.1]: `GetEmblem(recipient, abilities)` round-trips; the
    /// ability payload round-trips text → `GetEmblem` → text.
    #[test]
    fn get_emblem_round_trips() {
        let ron = "GetEmblem(You, [Static(Modify(It, Power(Up(1))))])";
        let v = read(ron);
        assert!(
            matches!(
                &v,
                Action::GetEmblem(Reference::You, a) if a.len() == 1
            ),
            "expected GetEmblem(You, [one ability]), got {v:?}"
        );
        let written = write(&v);
        assert_eq!(read(&written), v, "text → GetEmblem → text round-trips");
    }

    /// `ChangeLife(patient, Up(n))` writes/reads the patient explicitly —
    /// `You` is spelled, never omitted (Law 2).
    #[test]
    fn change_life_round_trips_you() {
        let v = Action::ChangeLife(Reference::You, LifeOp::Up(Count::Literal(1)));
        let written = write(&v);
        assert!(written.contains("You"), "the patient is spelled: {written}");
        assert_eq!(read(&written), v);
    }

    /// A non-`You` patient round-trips identically — no special-cased
    /// default branch.
    #[test]
    fn change_life_round_trips_other_patient() {
        let v = Action::ChangeLife(Reference::EventActor, LifeOp::Up(Count::Literal(3)));
        let written = write(&v);
        assert_eq!(read(&written), v);
    }
    ...
    /// `CastCopy(agent, CopySpec)` ([CR#707.12]) — "`agent` casts a copy of
    ...
    fn cast_copy_round_trips() {
        ...
        assert_eq!(read(&write(&cast)), cast, "CastCopy round-trips");
    }
    ...
    /// [CR#608.2g]: `Cast(agent, what, alt_cost)` — the agent is spelled
    /// (Law 2), "that card" is the surrounding effect's anaphor. The
    /// optional alternative-cost slot ([CR#118.9,702.35a]) stays a trailing
    /// tuple default: omitting it keeps `Cast(You, That(Card))` reading
    /// (Chandra), an explicit cost reads positionally `Cast(You,
    /// That(Card), [Mana(…)])` (madness) — the tuple/struct decision, whose
    /// deciding criterion is that bare read.
    #[test]
    fn cast_round_trips() {
        let v = Action::Cast(Reference::You, Reference::That(crate::Sort::Card), None);
        let written = write(&v);
        assert!(
        ...
        // An explicit, non-You caster reads native.
        let explicit = Action::Cast(Reference::It, Reference::It, None);
        assert_eq!(read("Cast(It, It)"), explicit);
        ...
        assert_eq!(
            read("Cast(You, That(Card), [Mana([Generic(1)])])"),
            for_cost,
