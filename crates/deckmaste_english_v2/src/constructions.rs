...
constructions! {
    ...
    construction quote_terminated_statement: AbilityBody {
        element QuoteTerminatedStatement {
            sentence: Sentence checked by sentence_ends_in_quoted_block(),
        }
...
fn sentence_ends_in_quoted_block(sentence: &Sentence) -> bool {
    match sentence {
        Sentence::Imperative(sentence) => predicate_ends_in_quoted_block(sentence.predicate()),
        Sentence::Declarative(sentence) => clause_ends_in_quoted_block(&sentence.clause),
        Sentence::Attached(sentence) => {
            clause_attachment_ends_in_quoted_block(&sentence.attachment)
        }
        Sentence::WithWhere(sentence) => match &sentence.clause {
            WhereClauseCategory::Where(clause) => {
                finite_clause_ends_in_quoted_block(&clause.clause)
            }
        },
    }
}

fn clause_attachment_ends_in_quoted_block(attachment: &ClauseAttachment) -> bool {
    match attachment {
        ClauseAttachment::StartingWith(attachment) => clause_ends_in_quoted_block(&attachment.body),
        ClauseAttachment::PreposedIf(attachment) => clause_ends_in_quoted_block(&attachment.body),
        ClauseAttachment::PreposedIfPredicate(attachment) => {
            predicate_ends_in_quoted_block(attachment.body())
        }
        ClauseAttachment::PreposedAs(attachment) => {
            predicate_ends_in_quoted_block(attachment.body())
        }
        ClauseAttachment::PreposedAsLongAs(attachment) => {
            clause_ends_in_quoted_block(&attachment.body)
        }
        ClauseAttachment::PreposedAsLongAsPredicate(attachment) => {
            predicate_ends_in_quoted_block(attachment.body())
        }
        ClauseAttachment::PreposedPredicateAdjunct(attachment) => {
            clause_ends_in_quoted_block(&attachment.body)
        }
        ClauseAttachment::PreposedPredicateAdjunctPredicate(attachment) => {
            predicate_ends_in_quoted_block(attachment.body())
        }
        ClauseAttachment::PreposedWhile(attachment) => {
            clause_ends_in_quoted_block(&attachment.body)
        }
        ClauseAttachment::PreposedWhilePredicate(attachment) => {
            predicate_ends_in_quoted_block(attachment.body())
        }
        ClauseAttachment::PreposedUntil(attachment) => {
            clause_ends_in_quoted_block(&attachment.body)
        }
        ClauseAttachment::PreposedUntilPredicate(attachment) => {
            predicate_ends_in_quoted_block(attachment.body())
        }
        ClauseAttachment::ThenSequence(sequence) => sequence
            .members()
            .last()
            .is_some_and(clause_ends_in_quoted_block),
        ClauseAttachment::AdditionalCost(attachment) => match attachment.body.as_ref() {
            AdditionalCostBody::Predicate(body) => predicate_ends_in_quoted_block(body.predicate()),
            AdditionalCostBody::Finite(body) => finite_clause_ends_in_quoted_block(&body.clause),
        },
        ClauseAttachment::PostposedIf(attachment) => {
            finite_clause_ends_in_quoted_block(&attachment.condition)
        }
        ClauseAttachment::PostposedIfPredicate(attachment) => {
            finite_clause_ends_in_quoted_block(&attachment.condition)
        }
        ClauseAttachment::PostposedUnless(attachment) => {
            finite_clause_ends_in_quoted_block(&attachment.condition)
        }
        ClauseAttachment::PostposedUnlessPredicate(attachment) => {
            finite_clause_ends_in_quoted_block(&attachment.condition)
        }
        ClauseAttachment::PostposedAsLongAs(attachment) => {
            finite_clause_ends_in_quoted_block(&attachment.condition)
        }
        ClauseAttachment::PostposedAsLongAsPredicate(attachment) => {
            finite_clause_ends_in_quoted_block(&attachment.condition)
        }
    }
}

fn clause_ends_in_quoted_block(clause: &Clause) -> bool {
    match clause {
        Clause::Finite(clause) => finite_clause_ends_in_quoted_block(clause),
        Clause::Coordination(coordination) => {
            let members = match coordination.as_ref() {
                ClauseCoordination::AndClauseCoordination(coordination) => coordination.members(),
                ClauseCoordination::OrClauseCoordination(coordination) => coordination.members(),
                ClauseCoordination::AndOrClauseCoordination(coordination) => coordination.members(),
            };
            members.last().is_some_and(|clause| match clause {
                CoordinatedClause::Finite(clause) => finite_clause_ends_in_quoted_block(clause),
            })
        }
        Clause::PostposedWhile(clause) => {
            let PostposedWhileClause::PostposedWhileClause(clause) = clause.as_ref();
            finite_clause_ends_in_quoted_block(&clause.condition)
        }
        Clause::PostposedForAsLongAs(clause) => {
            let PostposedForAsLongAsClause::PostposedForAsLongAsClause(clause) = clause.as_ref();
            finite_clause_ends_in_quoted_block(&clause.condition)
        }
    }
}

fn finite_clause_ends_in_quoted_block(clause: &FiniteClause) -> bool {
    match clause {
        FiniteClause::PlainFiniteClause(clause) => {
            predicate_ends_in_quoted_block(clause.predicate())
        }
        FiniteClause::PastPossessionClause(clause) => {
            matches!(clause.complement.as_ref(), PossessiveComplement::Quoted(_))
        }
        FiniteClause::ContractedPerfectTransitiveClause(clause) => {
            object_ends_in_quoted_block(&clause.object)
        }
        FiniteClause::ExistentialFiniteClause(clause) => {
            clause.adjunct.is_none() && noun_phrase_ends_in_quoted_block(&clause.pivot)
        }
        FiniteClause::FloatedQuantifierFiniteClause(clause) => {
            predicate_ends_in_quoted_block(clause.predicate())
        }
        FiniteClause::ThirdPersonAuxiliaryFiniteClause(clause) => {
            predicate_ends_in_quoted_block(clause.predicate())
        }
        FiniteClause::BareAgreementAuxiliaryFiniteClause(clause) => {
            predicate_ends_in_quoted_block(clause.predicate())
        }
        FiniteClause::ContractedCopularClause(_)
        | FiniteClause::ContractedPerfectObjectOnClause(_)
        | FiniteClause::ContractedPerfectPassiveClause(_) => false,
    }
}

fn predicate_ends_in_quoted_block(predicate: &Predicate) -> bool {
    match predicate {
        Predicate::Atomic(predicate) => verb_phrase_ends_in_quoted_block(predicate),
        Predicate::Coordination(coordination) => {
            predicate_coordination_ends_in_quoted_block(coordination)
        }
        Predicate::ThenSequence(sequence) => {
            let ThenPredicateSequence::ThenPredicateSequence(sequence) = sequence.as_ref();
            sequence
                .members()
                .last()
                .is_some_and(coordinated_predicate_ends_in_quoted_block)
        }
        Predicate::Auxiliary(predicate) => {
            let AuxiliaryPredicate::AuxiliaryPredicate(predicate) = predicate.as_ref();
            bare_predicate_ends_in_quoted_block(&predicate.predicate)
        }
        Predicate::Cause(predicate) => object_infinitive_ends_in_quoted_block(predicate),
        Predicate::Alternative(predicate) => alternative_ends_in_quoted_block(predicate),
        Predicate::WithoutGerundObject(predicate) => {
            without_gerund_object_ends_in_quoted_block(predicate)
        }
        _ => false,
    }
}

fn predicate_coordination_ends_in_quoted_block(coordination: &PredicateCoordination) -> bool {
    let members = match coordination {
        PredicateCoordination::AndPredicateCoordination(coordination) => coordination.members(),
        PredicateCoordination::OrPredicateCoordination(coordination) => coordination.members(),
        PredicateCoordination::AndOrPredicateCoordination(coordination) => coordination.members(),
    };
    members
        .last()
        .is_some_and(coordinated_predicate_ends_in_quoted_block)
...
fn coordinated_predicate_ends_in_quoted_block(predicate: &CoordinatedPredicate) -> bool {
    match predicate {
        CoordinatedPredicate::Atomic(predicate) => verb_phrase_ends_in_quoted_block(predicate),
        CoordinatedPredicate::Auxiliary(predicate) => {
            let AuxiliaryPredicate::AuxiliaryPredicate(predicate) = predicate.as_ref();
            bare_predicate_ends_in_quoted_block(&predicate.predicate)
        }
        CoordinatedPredicate::Cause(predicate) => object_infinitive_ends_in_quoted_block(predicate),
        CoordinatedPredicate::Alternative(predicate) => alternative_ends_in_quoted_block(predicate),
        CoordinatedPredicate::WithoutGerundObject(predicate) => {
            without_gerund_object_ends_in_quoted_block(predicate)
        }
        _ => false,
    }
...
fn bare_predicate_ends_in_quoted_block(predicate: &BarePredicate) -> bool {
    match predicate {
        BarePredicate::Atomic(predicate) => verb_phrase_ends_in_quoted_block(predicate),
        BarePredicate::Coordination(coordination) => {
            let members = match coordination.as_ref() {
                BarePredicateCoordination::BareAndPredicateCoordination(coordination) => {
                    coordination.members()
                }
                BarePredicateCoordination::BareOrPredicateCoordination(coordination) => {
                    coordination.members()
                }
                BarePredicateCoordination::BareAndOrPredicateCoordination(coordination) => {
                    coordination.members()
                }
            };
            members
                .last()
                .is_some_and(bare_coordinated_predicate_ends_in_quoted_block)
        }
        BarePredicate::ThenSequence(sequence) => {
            let BareThenPredicateSequence::BareThenPredicateSequence(sequence) = sequence.as_ref();
            sequence
                .members()
                .last()
                .is_some_and(bare_coordinated_predicate_ends_in_quoted_block)
        }
        BarePredicate::Cause(predicate) => object_infinitive_ends_in_quoted_block(predicate),
        BarePredicate::Alternative(predicate) => alternative_ends_in_quoted_block(predicate),
        BarePredicate::WithoutGerundObject(predicate) => {
            without_gerund_object_ends_in_quoted_block(predicate)
        }
        _ => false,
    }
...
fn bare_coordinated_predicate_ends_in_quoted_block(predicate: &BareCoordinatedPredicate) -> bool {
    match predicate {
        BareCoordinatedPredicate::Atomic(predicate) => verb_phrase_ends_in_quoted_block(predicate),
        BareCoordinatedPredicate::Cause(predicate) => {
            object_infinitive_ends_in_quoted_block(predicate)
        }
        BareCoordinatedPredicate::Alternative(predicate) => {
            alternative_ends_in_quoted_block(predicate)
        }
        BareCoordinatedPredicate::WithoutGerundObject(predicate) => {
            without_gerund_object_ends_in_quoted_block(predicate)
        }
        _ => false,
    }
...
fn object_infinitive_ends_in_quoted_block(predicate: &ObjectInfinitivePredicate) -> bool {
    let ObjectInfinitivePredicate::ObjectInfinitivePredicate(predicate) = predicate;
    verb_phrase_ends_in_quoted_block(&predicate.complement)
}

fn alternative_ends_in_quoted_block(predicate: &AlternativePredicate) -> bool {
    let AlternativePredicate::AlternativePredicate(predicate) = predicate;
    base_verb_frame_ends_in_quoted_block(&predicate.alternative)
}

fn without_gerund_object_ends_in_quoted_block(predicate: &WithoutGerundObjectPredicate) -> bool {
    let WithoutGerundObjectPredicate::WithoutGerundObjectPredicate(predicate) = predicate;
    object_ends_in_quoted_block(&predicate.complement)
}

fn verb_phrase_ends_in_quoted_block(predicate: &VerbPhrase) -> bool {
    match predicate {
        VerbPhrase::QuotedAbilityPredicate(_) => true,
        VerbPhrase::BaseVerbPhrase(predicate) => {
            base_verb_frame_ends_in_quoted_block(&predicate.frame)
        }
        VerbPhrase::ChooseInfinitivePredicate(predicate) => {
            let InfinitiveComplement::InfinitiveComplement(complement) =
                predicate.complement.as_ref();
            verb_phrase_ends_in_quoted_block(&complement.predicate)
        }
        VerbPhrase::HaveObjectControl(predicate) => {
            verb_phrase_ends_in_quoted_block(&predicate.predicate)
        }
        VerbPhrase::LookAt(predicate) => object_ends_in_quoted_block(&predicate.object),
        VerbPhrase::EnterControl(predicate) => object_ends_in_quoted_block(&predicate.control),
        VerbPhrase::DeclaredToObjectPredicate(predicate) => {
            object_ends_in_quoted_block(&predicate.complement)
        }
        VerbPhrase::DeclaredForObjectPredicate(predicate) => {
            object_ends_in_quoted_block(&predicate.object)
        }
        _ => false,
    }
}

fn base_verb_frame_ends_in_quoted_block(frame: &BaseVerbFrame) -> bool {
    match frame {
        BaseVerbFrame::TransitiveFrame(frame) => {
            let TransitiveFrame::TransitivePredicate(predicate) = frame.as_ref();
            object_ends_in_quoted_block(&predicate.object)
        }
        BaseVerbFrame::WithObjectFrame(frame) => {
            let WithObjectFrame::DeclaredWithObjectFrame(predicate) = frame.as_ref();
            object_ends_in_quoted_block(&predicate.object)
        }
        BaseVerbFrame::ObjectWithObjectFrame(frame) => {
            let ObjectWithObjectFrame::DeclaredObjectWithObjectFrame(predicate) = frame.as_ref();
            object_ends_in_quoted_block(&predicate.complement)
        }
        BaseVerbFrame::ObjectForObjectFrame(frame) => {
            let ObjectForObjectFrame::DeclaredObjectForObjectFrame(predicate) = frame.as_ref();
            object_ends_in_quoted_block(&predicate.complement)
        }
        BaseVerbFrame::ObjectIntoObjectFrame(frame) => {
            let ObjectIntoObjectFrame::DeclaredObjectIntoObjectFrame(predicate) = frame.as_ref();
            object_ends_in_quoted_block(&predicate.destination)
        }
        _ => false,
    }
}

fn object_ends_in_quoted_block(object: &Object) -> bool {
    let Object::ObjectNominal(object) = object else {
        return false;
    };
    noun_phrase_ends_in_quoted_block(&object.value)
}

fn noun_phrase_ends_in_quoted_block(noun_phrase: &NounPhrase) -> bool {
    let NounPhrase::QualifiedNounPhrase(noun_phrase) = noun_phrase else {
        return false;
    };
    matches!(
        noun_phrase.reference.as_ref(),
        NumericStage::GrantedAbilityQualifiedReference(reference)
            if matches!(reference.granted.as_ref(), GrantedAbility::Quoted(_))
    )
}
