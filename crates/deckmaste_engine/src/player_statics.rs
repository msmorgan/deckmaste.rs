...
//! (`Up(LandPlaysPerTurn, 1)`, [CR#305.2]) and Reliquary Tower
...
/// seam — it contributes `0` to an `Up`/`Down` and an unevaluated `0` to a
/// `Set`, which is the conservative no-op until the `ExecutionFrame`-bearing evaluator
/// is threaded in.
...
impl GameState {
    ...
    /// (a removed cap has no value); otherwise `Set` overwrites and
    /// `Up`/`Down` add and subtract.
    #[must_use]
    pub fn effective_player_attr(
        ...
        self.for_each_player_mod(|target, pmod| {
            ...
            match pmod {
                PlayerMod::Set(a, n) if *a == attr => value = literal(n),
                PlayerMod::Up(a, n) if *a == attr => value += literal(n),
                PlayerMod::Down(a, n) if *a == attr => value -= literal(n),
...
mod tests {
    ...
    /// Exploration: `Up(LandPlaysPerTurn, 1)` lifts the controller to two
    ...
    fn exploration_raises_land_plays_for_controller_only() {
        ...
        modify_player_permanent(
            &mut state,
            PlayerId(0),
            PlayerMod::Up(PlayerAttr::LandPlaysPerTurn, Count::Literal(1)),
        );
