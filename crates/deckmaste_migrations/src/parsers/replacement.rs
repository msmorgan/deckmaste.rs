...
fn subtype_atom(word: &str) -> Option<String> {
    ...
    Some(format!("Subtype({})", crate::ident::to_rust_ident(word)))
}
...
mod tests {
        ...
        assert_eq!(
            ...
            Some(
                "Static(Replacement(AsEnters(Continuously(effect: \
                 Modify(This, Several([Power(Up(1)), Toughness(Up(1))])), \
    ...
    fn unless_control_a_subtype() {
        assert_eq!(
            ...
            Some(
                tapped_unless("Exists(And([Permanent, Subtype(Swamp), ControlledBy(Ref(You))]))")
                    .as_str()
            )
        );
    }
