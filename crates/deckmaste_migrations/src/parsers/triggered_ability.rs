...
fn parse_event(clause: &str) -> Option<String> {
    ...
    let (subject, event) = if let Some(subject) = clause.strip_suffix(" enters") {
        (subject, "Enters")
    ...
    Some(format!("{event}({})", event_subject(subject)?))
}

/// A trigger subject phrase -> its `Filter` RON. `~` is the source object;
/// "a creature" is any creature.
fn event_subject(subject: &str) -> Option<&'static str> {
    match subject {
        "~" => Some("Is(This)"),
        "a creature" => Some("Type(Creature)"),
        _ => None,
    }
}
...
mod tests {
    ...
    fn etb_self_draw() {
        assert_eq!(
            ...
            Some("Triggered(event: Enters(Is(This)), effect: Draw(1))")
        );
    }
    ...
    fn dies_self_targeted_damage() {
        assert_eq!(
            ...
            Some(
                "Triggered(event: Dies(Is(This)), targets: [AnyTarget], effect: DealDamage(Target(0), 1))"
            )
        );
    }
    ...
    fn whenever_a_creature_dies_lose_life() {
        assert_eq!(
            ...
            Some("Triggered(event: Dies(Type(Creature)), effect: LoseLife(1))")
        );
    }
    ...
    fn whenever_a_creature_enters_draw() {
        assert_eq!(
            ...
            Some("Triggered(event: Enters(Type(Creature)), effect: Draw(1))")
        );
    }
