...
mod tests {
    ...
    fn creature_trigger_resolves_through_registry() {
        ...
        assert!(matches!(
            &face.abilities[0],
            TodoAbility::Parsed(r)
                if r == "Triggered(event: ThisDies, effect: Targeted(targets: [AnyTarget], effect: DealDamage(This, 1, Target(0))))"
        ));
    }
    ...
    fn permanent_static_anthem_resolves() {
        ...
        assert!(matches!(
            &face.abilities[0],
            TodoAbility::Parsed(r)
                if r == "Static(Each(SelectAll(And([Creature, ControlledBy(Ref(You))])), Modify(It, Several([Power(Up(1)), Toughness(Up(1))]))))"
        ));
    }
