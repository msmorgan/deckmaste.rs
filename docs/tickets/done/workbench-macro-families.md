...
- Merged onto the landed pronoun-read, own-read, and effect-axes rounds: `LookReq`, `agentLookedTop`, `agentMovedRest`, and the `scry`/`surveil` gates are re-spelled over `countReach`/`tyOfReach`, their agent clauses match `Pro (Word|UnionHalf|Verbed … PlayerW) _` and `AttachHost _ PlayerW`, rider-less `Move`s spell `[]` where the round wrote `MkMoveRiders`, and `proliferate` inlines its group over `countReach (Word JoinW) ManyOf` with `PutCounters (Lit 1) OwnKinds`.
- The three counter witnesses both sides touched carry `PrintedKind` inside the round's `May`/`IfDone` spellings; the own-read pins `ownSurvivesSecondSingular` and `badItAcrossOwnSlot` read through `exile You`; `noRiders`, `preventNextBy`, `itsAnAbility`, `discardN`, and the other landed re-spellings of deleted macros stay deleted.
