...
- Merge: composed the determiner-slot and kind-join spellings with the slot-fold possessor, combat, counter-axis, folded-row, and face spellings across Cards/Proofs; kept Phrase's landed `nounDet` projections and combined `Macros.counted` with the new counter axes in `proliferate`.
- Merge fix: re-spelled the three new `TokensCreated` pins through `Macros.counted`; their valid twins (causer only, singular controller, and non-minting `You`) each made the compiler reject `Oh impossible`, confirming non-vacuity after the merge.
