...
Measured feature `lolyrvsq`, based on claim `mmksxwql`; matched baseline tree
`mmksxwql` (the coordinator was its empty child when measured). Use that claim
revision to reproduce the baseline, not the moving coordinator working copy.
Kata's final refresh was a no-op. The v2 coverage lock is
...
|---|---:|---:|---:|---:|
| Baseline `mmksxwql`, text | 30,957 | 1,166 | 445 | 0 |
...
The capitals are sentence-initial surface variants generated by lexical from
the declared `Capitalization::Initial` policy, not duplicate declarations or
separate proper names. This is one noun–verb Lexeme pair across four
case-sensitive spellings; `SurfaceCase` retains the spelling for realization.
...
baseline text `mmksxwql` **9.600 s**, **38,330 ns/B**, host load
