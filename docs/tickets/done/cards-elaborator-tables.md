...
needs: []
---
**The load-time elaborator (`deckmaste_plugin::elaborate`) + the Idris table
...
`deckmaste_plugin::elaborate(card) -> Result<ElabCard, ElabError>`: one walk, one
...
`crates/deckmaste_plugin/tests/reject/` that must fail with exactly that code.
...
- `deckmaste_plugin::elaborate` walks every card of a loaded plugin; `cargo
...
- `cargo test -p deckmaste_plugin` — reject fixtures each fail with their code.
