...
pipeline. Loud residue: non-mana components on a cost-change row, in both
`add_mana_components` and `reduce_mana_components` — owned by
`engine-cost-modification-residue`; the optional-`Additional` (kicker, [CR#118.8b])
