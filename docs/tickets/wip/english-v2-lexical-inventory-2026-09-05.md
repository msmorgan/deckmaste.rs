...
- **Baseline caveat (reviewer).** The parent side below is not stamped with a
  change id and was not built at this feature's fork point. Per the coordinator
  amendment of 2026-09-05, a before/after census must compare two trees measured
  with IDENTICAL flags (`--require-resolved` materializes extra candidates) and
  the parent side must be the fork point checked out in-workspace via a
  temporary `jj new <fork>`, never DEFAULT's checkout. Trunk has since moved
  (`english-v2-affinity-quality-surface` integrated, covered 19,622), so every
  corpus figure in this record must be re-derived after a refresh before the
  batch lands.
...
- Performance advisory, workers 8 throughout: coverage `--check` 142,293 ms,
  ...
  load and is reported, not fitted to. **Contention stamp (reviewer):** the
  implementer's measurements ran against 3 concurrent codex executors plus 1
  other Opus landing reviewer on the host, so every figure above is a
  loaded-host figure and none of them bounds the quiet-host ceiling.
- Assurance counts (corrected in review): re-spelled 4 existing assertions
  (the modal negative flipped to a positive witness and its test renamed; the
  nursery aggregate 195 -> 200; the declaration-term aggregate 196 -> 201; the
  quoted-interior first-failure attribution `shares` -> `feelings`); ignored 0;
  added 0; removed 0. The original line's "restored 11 payload paths" is not
  reproducible against the diff and has been withdrawn.
...
The review raised three HIGH findings and reported REJECTED-PENDING-RULING with
two questions. The coordinator ruled the same day; both rulings are recorded
verbatim under the STOP below, and both were executed in review commit
`review: drop the fused-landwalk and exert rows; mint their structural tickets`.
Every figure in PROVE / DISCLOSE / REPORT above is the implementer's
pre-ruling measurement and is **superseded** by the restamped section at the end
of this record.

#### H1 (fixed) — a generated, gitignored catalog had been hand-edited

`data/gen/catalogs/keyword-abilities.txt` is generated by
`crates/deckmaste_catalogs/src/cr.rs` (`parse_headings(&lines, 702, "Keyword
Abilities", …)`) from `data/rules/cr.txt`, and it is the sole authority the
nursery test compares the builtin-v2 stub set against
(`crates/deckmaste_construction_core/tests/builtin_v2_keyword_abilities.rs`,
`assert_eq!(actual.keys(), expected.keys())`). `data/` is a symlink into the
coordinator checkout, so the file is shared by every workspace and a hand edit
there is invisible to `jj diff`, invisible to `jj st`, and survives `kata
refresh`.

The five fused landwalk stubs and the `assert_eq!(actual.len(), 200)` were green
only against a 200-head version of that file, which no generator produces: the
canonical generator emits **195** heads with `Landwalk` as the only walk head,
and the legacy AtomicCards-derived generator (`data/gen/catalogs-legacy/`) emits
**220** with **nine** walk heads — `Desertwalk`, `Forestwalk`, `Islandwalk`,
`Landwalk`, `Legendary landwalk`, `Mountainwalk`, `Nonbasic landwalk`,
`Plainswalk`, `Swampwalk`. 200 is 195 plus exactly the five basic-land fusions
and nothing else.

#### H2 (fixed by ruling) — the fused-landwalk row is dropped

The generator repair the ticket sketched is refused by a standing ruling encoded
as a test:
`crates/deckmaste_catalogs/src/lib.rs::canonical_keyword_abilities_exclude_all_legacy_atomic_variants`
asserts `"Islandwalk"` must **not** appear in the canonical catalog. The CR does
not pick out five either — `[CR#702.14a]` makes `[type]` "usually a land type,
but it can also be the card type land plus any combination of land types, card
types, and/or supertypes", `[CR#205.3i]` lists seventeen land types and marks
five of them basic, and AtomicCards prints `Desertwalk` (2), `townwalk` (1),
`Planeswalkerwalk` (1) and `Denimwalk` (1) besides the five basics.

```text
**Coordinator ruling, 2026-09-05 (Q1) — verbatim.** "Q1 = (a). The canonical
keyword-ability catalog stays the CR-702 headings; the `Islandwalk`-excluded
test stands. Drop the fused-landwalk row entirely: remove the five stubs,
restore the 195 counts in builtin_v2_keyword_abilities.rs and xtask facts.rs,
regenerate idris/src/Experimental/FactsGen.idr, confirm `cargo xtask catalogs
check` clean (the shared file is already trunk's 195). The 146 identities are
an EXPLAINED loss: re-coverage owed to a new STRUCTURAL ticket you mint at
docs/tickets/planned/english-v2-landwalk-quality-keyword.md (needs: []): a
keyword-ability construction reading the `Landwalk` declaration with a
land-type quality, realized as the fused surface `<Type>walk` ([CR#702.14a]
"[type]walk") and the separated forms `Nonbasic landwalk` / `Legendary
landwalk` ([CR#702.14b], [CR#702.14c] — verify the sub-rule letters against
cr.txt before citing), covering all printed walk surfaces including
Desertwalk; the land-type quality is read from the type declarations, never a
list of the five basic words; cite `docs/keyword-policy.md` §4's v1 shape and
note that the `macro-keyword-templates` ruling ('typed landwalk stays bare
keyword names') governs v1 macro templates and is not a constraint on the v2
grammar — if you read it as a constraint on v2, say so in the ticket as an
open question rather than resolving it. Expected units: the 146 plus
Desertwalk/nonbasic/legendary printings (count them with jq). Tier note in the
ticket: sol."
```

Executed. The five stubs are deleted, the nursery count is back to 195, the
`declaration_terms` aggregate back to 196, xtask's five facts rows and its "all
195 stubs" prose restored, and `idris/src/Experimental/FactsGen.idr` regenerated
by `cargo xtask facts generate` (197 rows) so that it is byte-identical to the
fork point. `cargo xtask catalogs check` reports `catalogs are up to date` and
`cargo xtask facts check` reports the module up to date.
`docs/tickets/planned/english-v2-landwalk-quality-keyword.md` is minted, with
the sub-rule letters corrected against `data/rules/cr.txt`: `[CR#702.14b]` is
only "Landwalk is an evasion ability", so the separated forms cite
`[CR#702.14a,702.14c]` (the latter gives "artifact landwalk" / "nonbasic
landwalk" / "snow swampwalk" by example). Counted with `jq`: `Desertwalk` 2, `Nonbasic
landwalk` 3, `Legendary landwalk` 2 printings, plus the joke surfaces `townwalk`
1, `Planeswalkerwalk` 1, `Denimwalk` 1 noted out of scope.

#### H3 (fixed by ruling) — the `exert` row is dropped

`plugins/builtin_v2/macros/stubs/keyword_actions/Exert.ron` already owns `exert`
and records `third_person: Unavailable` with the note "No Oracle or
Comprehensive Rules occurrence of `exerts` was found in the 2026-08-18
morphology audit". `core_verbs.ron`'s schema makes `third_person` mandatory
(`crates/deckmaste_english_v2/src/environment.rs`, `third_person: String`), so
homing `exert` there required inventing `"exerts"` — a form with zero
occurrences in AtomicCards. The owner map in `reject_literal_lexicon_collisions`
is first-wins (`or_insert_with`), so two owners for one surface passed silently.

Root cause, verified by renaming only the core verb's surfaces: a keyword-action
verb declared with `third_person: Unavailable` contributes **no verb reading at
all**. `{T}, Exert this creature: Draw a card.` then reports `outcome
status=parse_failure span_start=5 span_end=10`; on the unmodified tree `Goad
target creature.` and `Detain target creature.` fail at the verb, while `Destroy
target creature.` (no `Unavailable`) selects.

```text
**Coordinator ruling, 2026-09-05 (Q2) — verbatim.** "Q2: drop the `exert` row
(CoreVerbIdentity::Exert and its surfaces) — Exert.ron owns the lexeme, and
the duplicate authority is a defect on its own. The root cause is a ruling
violation, not a data gap: `third_person: Unavailable` withholds a grammatical
English inflection because it is unattested, which is attestation used as a
filter (rewrite ADR, 2026-09-04: attestation is provenance, never a filter).
Mint docs/tickets/planned/english-v2-keyword-action-verb-inflections.md
(needs: []): every keyword-action verb declaration contributes the full finite
paradigm derived by the compiler's morphology (third-person, preterite,
participle) with `Unavailable` retired as a licence — a form's absence from
AtomicCards is recorded as provenance in the report, never as a gate; name the
17 affected stubs by grepping, state the expected effect on the exert/other
identities this row was covering, and cite [the exert rule] for exert only after
checking the number in cr.txt. Any identities the `exert` row covered are an
explained loss owed to that ticket."
```

One editorial substitution in that quotation: the ruling wrote a placeholder
rule number where "[the exert rule]" now stands, because the citation checker
reads a placeholder as a malformed citation. The actual rule, read from
`data/rules/cr.txt`, is `[CR#701.43a]`.

Executed. `CoreVerbIdentity::Exert`, its `core_verbs.ron` row and its four
`environment.rs` sites are removed and `ALL` is back to 41.
`docs/tickets/planned/english-v2-keyword-action-verb-inflections.md` is minted,
naming all seventeen stubs (`Abandon`, `Behold`, `Bolster`, `CollectEvidence`,
`Detain`, `Exert`, `Fateseal`, `Forage`, `Goad`, `Harness`, `Learn`,
`Monstrosity`, `Populate`, `Recruit`, `Support`, `Suspect`, `TimeTravel`), the
six identities the row was covering, and `[CR#701.43a]` — the rule number read
from `data/rules/cr.txt`, not from the placeholder in the ruling.

#### M1 (fixed) — the `unlock` residue is routed to a live owner

The row gained zero identities and its residue had been routed to
`workbench-room-halves`, which is in `docs/tickets/done/`. A done ticket is not
a plan. The substance — the `door` noun and the half-level referent, with the
verb now proven not to be the blocker (the parse advances through `unlock` and
stops at bytes 21..25 on `door`) — is recorded as **Door half-level trigger** in
`docs/tickets/fog.md`. The `unlock` member itself stays: it is correct data and
its third person `unlocks` is attested 56 times.

#### M2, M3, M4 (fixed) — record defects

The assurance line's unverifiable "restored 11 payload paths" is withdrawn; the
census carries a fork-point baseline caveat; the contention stamp is written in.
All three are applied above and superseded by the restamped section below.

#### LOW (not fixed) — redundant assertion

`crates/deckmaste_english_v2/tests/ability_logic.rs` re-asserts
`selected.render(...) == text` immediately after `assert_one_logic_candidate`,
which already asserts byte-exact render. Harmless; left as authored so the
witness reads standalone.

#### What the review verified as sound and kept

- The authorized negative-to-positive flip is correct: the re-spelled witness
  goes through `assert_one_logic_candidate`, which asserts selection, exactly
  one materialized candidate, byte-exact render, complete byte ownership and
  zero ownership failures. The style-guide sentence quoted in the test is real
  (`docs/oracle-style-guide.md`, "Modal choices") and Inscription of Abundance,
  Insight and Ruin print the surface.
- The quoted-interior negative still rejects; only its first-failure
  attribution moved, and the test's subject is intact.
- The `kicked` row is homed correctly on the `Kicker` keyword-ability
  declaration's `participial_adjective`, not on a core verb.
- No `require`, `checked by`, Rust predicate or comment in the diff names a
  lexeme, construction, verb, noun, preposition or card identity.
- Third-person forms checked against the corpus: `unlocks` 56, `shares` 92,
  `wins` 41 — all attested; only the retired `exerts` was 0.

### RESTAMPED — the reviewed tree

Everything in this section supersedes the pre-ruling figures above. All figures
are measured on the tree of review change `lqoslwnp` (the reviewed feature tip)
after `kata refresh` onto the moved trunk, with lock `covered = 19,845`.

#### Baseline discipline

The parent side is this feature's **fork point**, change `vnznstuq` (the claim
commit, rebased by the refresh), checked out in-workspace with `jj new
vnznstuq` and abandoned afterwards — not the coordinator checkout, which drifts
...
| selected | 19,622 | **19,845** |
| unique | 15,560 | **15,678** |
| specificity-resolved | 4,062 | **4,167** |
| unresolved ties | 0 | 0 |
| internal failures | 0 | 0 |
| parse failures | 13,019 | 12,796 |

#### PROVE

- **No silent loss.** The exact per-unit diff of the two runs above: **223
  gains, 0 losses, 0 changed selected construction paths, 0 changed resolution
  kinds**, and the identity sets are equal on both sides (no id present on one
  side only). The lock moved 19,622 -> 19,845, **+223 / -0**, and the lock diff
  is 223 insertions / 0 deletions. `coverage --check` emitted `newly covered 223
  corpus identities` and no no-longer-covered section.
- **The two dropped rows are explained losses, not silent ones.** Relative to
  the implementer's pre-ruling measurement the count falls 375 -> 223: 146
  fused-landwalk identities, owed to
  `docs/tickets/planned/english-v2-landwalk-quality-keyword.md`, and 6 `exert`
  identities, owed to
  `docs/tickets/planned/english-v2-keyword-action-verb-inflections.md`. Both are
  wrong-analysis retirements with a named live owner; neither is a regression.
  Against the fork point nothing at all stopped being covered.
- **Structural laws.** `coverage --check`: 32,641 total units, 19,845 selected
  and covered, 0 selected-uncovered, 0 unresolved ties, 0 internal failures, 0
  exception resolutions or uses, 0 roundtrip mismatch units, 0 ownership failure
  units, 0 traversal failure units, 0 leaf-traversal failure units, 0 gap spans
  and 0 overlap spans. Construction and leaf traversal identity exact: 883,345
  expected nonterminal nodes = 883,345 visited constructions; 308,280 expected
  leaves = 308,280 visited leaves. `roundtrip --require-clean`: 19,845 parse
  accepted, 19,845 clean, 0 mismatched, 12,796 not accepted.
- **No word-naming.** `licensing_checker_permitted` 23,
  `licensing_checker_forbidden` **0**; the `environment.rs` load checks are
  green (the parser environment freezes, so every declaration and catalog
  provider loaded).
- **The batch's defining acceptance holds**: the construction declaration count
  is unchanged, and the diff adds no construction, sum, seam, feature, frame
  family or guard. What remains is three `core_verbs.ron` rows, one
  `participial_adjective` on an existing keyword-ability stub, and their
  `environment.rs` inventory entries.

#### DISCLOSE

- **Newly covered identities and their selected analyses.** All 223 gains were
  read from their selected construction paths, not from the lock's card-text
  column, and every one is `selected` (no negative oracle, no wrong analysis):
  - `kicked` **139** — through
    `ParticipialAdjectiveDeclaredParticipialAdjective` /
    `PredicativeParticipialAdjectiveComplementPredicativeParticipialAdjective`,
    i.e. the `Kicker` declaration's own participial adjective, under
    `PreposedIfClauseTailPreposedIfClauseTail` (75) or
    `FiniteConditionFiniteCondition` (42) or both (7), plus 15 through the
    participial nominal modifier and the modal-mode header.
  - `share` / `shares` **65** — through
    `FiniteSubjectGapRelativeClauseFiniteTransitiveSubjectGapRelativeClause`,
    with 7 also carrying an intransitive predicate and 7 through
    `UnqualifiedReferenceParticipialSingularReference`.
  - `win` **19** — through
    `IntransitiveLexicalVerbPhraseIntransitivePredicate` under
    `PreposedIfClauseTailPreposedIfClauseTail`, and through
    `PredicateCoordinationAndPredicateCoordination` for `clash and win`.
  - `unlock` **0**, `exert` **0** (row retired), fused landwalk **0** (row
    retired). Every one of the 223 attributes to a surviving row by surface;
    none is unattributed and none carries a walk surface.
- **Selection census.** unique 15,560 -> 15,678, specificity-resolved 4,062 ->
  4,167. The specificity share rose because **105 of the 223 gains materialize
  more than one candidate** — the rise is exactly the gains, with no
  previously-selected unit changing resolution kind. The contributing
  construction pairs are `FiniteCondition` over `PreposedIfClauseTail` for the
  `kicked` and `win` gains, and the subject-relative / prepositional qualified
  reference nesting rivalry for the `share` gains.
- **Deviations and additions.** Against the ticket's letter, this landing now
  delivers four of its six rows and retires two by coordinator ruling. Beyond
  the ticket: one existing modal negative assertion re-spelled as the positive
  witness (authorized); one quoted-interior negative's first-failure
  attribution re-spelled from `shares` to `feelings`; two structural tickets
  minted; one fog entry added. Nothing else was added or deleted — no
  construction, test, sum, seam, feature, frame family or guard.
- **STOPs.** One raised by the implementer (the modal negative) and resolved by
  coordinator ruling; two raised by the review (Q1, Q2) and resolved by
  coordinator ruling, both recorded verbatim above. None resolved unilaterally.
- **Glossary gap.** `docs/contexts/oracle-english/CONTEXT.md` defines no term
  for the *fused keyword surface* — a keyword ability whose parameter is glued
  to the keyword stem as one orthographic word (`islandwalk`,
  `plainscycling`). The concept is now load-bearing for
  `english-v2-landwalk-quality-keyword`; naming it is left to that ticket rather
  than resolved here, so the gap is recorded, not filled.

#### REPORT

- Coverage lock: 19,622 -> **19,845**, **+223 / -0**, on review change
  `lqoslwnp`. Construction declarations unchanged.
- Licensed vocabulary/lexeme homographs, reported not fitted: 2 —
  `AttributiveAdjective::Untap` beside the keyword-action verb declaration
  `Untap`; `TargetingMarker::Target` beside noun lexeme `CommonNoun::Target`.
  The retired `exert` row would have added a third; it is gone.
- Form-literal/vocabulary overlaps, reported not fitted: 9 — `additional` at
  ...
  and `next` at `definite_next_mass_quantity_reference` atoms 0 and 1; `to` at
...
- Gate: `cargo xtask gate --changed` printed
  `cargo test -p deckmaste_construction_core -p deckmaste_construction -p deckmaste_english_v2 -p xtask`,
  and `--run --clippy` ran that plus
  `cargo clippy -p deckmaste_construction_core -p deckmaste_construction -p deckmaste_english_v2 -p xtask --all-targets -- -D warnings`;
  both clean, 0 failed suites. `cargo fmt --all` clean. `cargo xtask catalogs
  check` and `cargo xtask facts check` both up to date.
- Assurance counts: restored 0; re-spelled 4 existing assertions (the modal
  negative flipped to a positive witness and its test renamed; the nursery
  aggregate, restored to 195 in review; the declaration-term aggregate, restored
  to 196 in review; the quoted-interior first-failure attribution `shares` ->
  `feelings`); ignored 0; added 0; removed 0. The one `ignored` test in the
  closure run is pre-existing and untouched by this diff.
- Performance advisory, workers 8 throughout, all four passes on the reviewed
  tree: fork-point ambiguity 107,751 ms, 125,838 ns/B, host load 7 / 9 / 10;
  feature ambiguity 114,732 ms, 147,696 ns/B, host load 7 / 9 / 10; coverage
  `--check` 108,822 ms, 124,899 ns/B, host load 5 / 8 / 9; roundtrip 106,976
  ms, 126,013 ns/B, host load 8 / 8 / 9. The 16,260 ms quiet-host ceiling was
  exceeded on every pass and is reported, not fitted to. **Contention stamp:**
  the implementer's own measurements (above, superseded) ran against 3
  concurrent codex executors plus 1 other Opus landing reviewer; the review
  passes above ran at host load 5-10 with sibling workspaces still active. No
  figure here bounds the quiet-host ceiling.
- Citation changes: `[CR#702.14a]`, `[CR#702.14b]` and `[CR#702.14c]` newly
  registered by `cargo xtask cite bless`; `[CR#205.3i]` and `[CR#701.43a]` were
  already registered. `cargo
  xtask cite check --list-noncompliant` empty, `cite check` 0 stale, and every
  site in `jj diff --git | cargo xtask cite audit --diff` read against its
  claim. Decisions wanted: none.
