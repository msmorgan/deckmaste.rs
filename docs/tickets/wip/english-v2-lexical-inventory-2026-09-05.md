...
**Verdict: REJECTED-PENDING-RULING.** Not integrated. Two HIGH findings need a
coordinator ruling before any part of this batch reaches trunk; the record
corrections above are applied, the rest are stated as findings.

#### H1 — a generated, gitignored catalog was hand-edited (duplicated authority)
...
Abilities", …)`) from `data/rules/cr.txt`, and it is the single authority the
nursery test compares the builtin-v2 stub set against
(`crates/deckmaste_construction_core/tests/builtin_v2_keyword_abilities.rs:180`,
`assert_eq!(actual.keys(), expected.keys())` with `expected` read from that
file). `data/` is a symlink into the DEFAULT checkout, so the file is shared by
every workspace.

The landing's five fused landwalk stubs and its `assert_eq!(actual.len(), 200)`
were green only against a 200-head version of that file. No generator produces
200 heads:

- the canonical generator emits **195**, with `Landwalk` as the only walk head
  (`cargo xtask catalogs check` on the regenerated file: `catalogs are up to
  date`);
- the legacy AtomicCards-derived generator (`data/gen/catalogs-legacy/`) emits
...
200 is 195 plus exactly the five basic-land fusions and nothing else, which is
neither generator's output. The feature diff touches no `deckmaste_catalogs`
file. Against the CR-generated catalog the landing is RED:

```
test builtin_v2_keyword_ability_nursery_is_complete_and_normalized ... FAILED
assertion `left == right` failed
  left:  [… "Forestwalk" … "Islandwalk" … "Mountainwalk" … "Plainswalk" … "Swampwalk" …]  (200)
  right: [… no fused landwalk heads …]                                                    (195)
```

#### H2 — the fix direction contradicts a recorded ruling (RULING NEEDED)

The obvious repair — teach `deckmaste_catalogs` to derive the fused heads from
`[CR#702.14a]` and `[CR#205.3i]` — is refused by a standing ruling encoded as a
test in that same crate:

`crates/deckmaste_catalogs/src/lib.rs::canonical_keyword_abilities_exclude_all_legacy_atomic_variants`
asserts that `"Islandwalk"` (with `Basic landcycling`, `Friends forever`,
`Hexproof from`, `Choose a Background`) **must not** appear in the canonical
keyword-abilities catalog. Deriving the fused heads means deleting or inverting
that assertion, which is a ruling reversal, not a data line.

The CR does not pick out five, either. `[CR#702.14a]` defines `[type]walk` where
`[type]` is "usually a land type, but it can also be the card type land plus any
combination of land types, card types, and/or supertypes"; `[CR#205.3i]` lists
seventeen land types and marks five of them basic. The printed corpus agrees
with the CR and not with the landing: besides the five basics AtomicCards prints
`Desertwalk` (2), `townwalk` (1), `Planeswalkerwalk` (1) and `Denimwalk` (1).
So "the five fused landwalk keywords" is a chosen subset, i.e. a design
dimension the ticket did not pin.

**Question for the coordinator (either answer is implementable; the reviewer
will not choose):**

> The canonical keyword-ability inventory is defined as the Comprehensive
> Rules section-702 headings and deliberately excludes fused landwalk variants as *legacy atomic
> variants*. Should english-v2 (a) keep that exclusion, drop this row, and route
> the fused landwalk surfaces to a construction that reads `Landwalk` plus a
> land-type quality (the shape `macro-keyword-templates` and `keyword-policy.md`
> §4 already describe for v1, `quality: Default(Predicate, Type(Land))`); or
> (b) redefine the canonical catalog to admit printed fused keyword surfaces,
> retiring `canonical_keyword_abilities_exclude_all_legacy_atomic_variants` and
> covering all nine printed walk surfaces rather than five?

#### H3 — `CoreVerbIdentity::Exert` mints a surface the repo has recorded as nonexistent

`plugins/builtin_v2/macros/stubs/keyword_actions/Exert.ron` already declares
`exert` as a transitive verb, and records `third_person: Unavailable` with the
note "No Oracle or Comprehensive Rules occurrence of `exerts` was found in the
2026-08-18 morphology audit". `core_verbs.ron`'s schema makes `third_person`
mandatory (`crates/deckmaste_english_v2/src/environment.rs:207`,
`third_person: String`), so homing `exert` there **requires** inventing
`"exerts"` — a surface with zero occurrences in AtomicCards. That is a
fabricated licensed inflection and a second owner for one lexeme; the owner map
in `reject_literal_lexicon_collisions` is first-wins (`or_insert_with`), so the
environment cannot detect the duplication.

Root cause the row is working around: a keyword-action verb declared with
`third_person: Unavailable` contributes **no verb reading at all**. Verified by
renaming only the core verb's surfaces, after which `{T}, Exert this creature:
Draw a card.` reports `outcome status=parse_failure span_start=5 span_end=10`;
`Goad target creature.` and `Detain target creature.` fail at the verb on the
unmodified tree, while `Destroy target creature.` (no `Unavailable`) selects.
Seventeen keyword-action stubs carry `third_person: Unavailable`.

**This needs a ruling too:** the correct home for `exert` is the declaration
that already owns it, and making it reachable is a compiler/declaration fix
outside a "data line" batch. Either the row is dropped and the defect routed to
a new ticket, or the ticket is widened to fix `DerivedSurface::Unavailable`
verbs. The reviewer will not choose between shipping a fabricated inflection and
silently widening the batch.

#### M1 — the `unlock` row is inert and routed to a closed ticket

The record's own table gives `unlock` **0** gained identities, and the ticket
states that a row gaining nothing was misdiagnosed. Its outstanding work is
routed to `workbench-room-halves`, which is in `docs/tickets/done/`. Ledger
residue must route to a **live** ticket; a done ticket is not a plan. Either
drop the row with the rest of the misclassified batch, or mint a live ticket
before it lands.

#### M2 — assurance line corrected

See REPORT above: "restored 11 payload paths" is not reproducible against the
diff and has been withdrawn.

#### M3 — census baseline not fork-point-stamped

See DISCLOSE above.

#### M4 — contention stamp added

See REPORT above.

#### LOW — redundant assertion
...
which already asserts byte-exact render at `ability_logic.rs:2500`.
...
- The authorized negative-to-positive flip is correct: the re-spelled witness
  ...
  (`docs/oracle-style-guide.md:1477`) and Inscription of Abundance prints the
  surface.
- The quoted-interior negative still rejects; only its first-failure
  attribution moved, and the test's subject is intact.
- Lock arithmetic matches the record exactly: `covered` array 19,939, 375
  identities added, **0 removed** — no silent loss.
- Selected analyses spot-checked from construction paths, not from the lock's
  card-text column: `shares` selects through
  ...
  `win` through `PreposedIfClauseTailPreposedIfClauseTail` and through
  `PredicateCoordinationAndPredicateCoordination`; `kicked` through
  `PredicativeParticipialAdjectiveComplementPredicativeParticipialAdjective` /
  `ParticipialAdjectiveDeclaredParticipialAdjective` — i.e. owned by the
  `Kicker` keyword-ability declaration's `participial_adjective`, the correct
  home; `Exert` through `ActivationCostComponentCostClause`. All render
  byte-exact.
...
- `unlocks` (56), `shares` (92) and `wins` (41) all occur in AtomicCards; only
  `exerts` (0) is fabricated.
- No other generated input under `data/gen` was hand-edited: `cargo xtask
  catalogs check` reports `catalogs are up to date`, and nothing under
  `data/gen/english-v2/` or `data/gen/catalogs-legacy/` was written during the
