...
- **Routed from workbench-event-zone-3-targeting-and-disjunction-arms (close, 2026-08-26):** bare "it" after a becomes-target header is refused — `eventAfter` announces both participants, so `countOnes Object = 1` fails when the targeter is object-kinded; costs "…, sacrifice it" (18 lines, the largest body family). The proposed fix is an anaphora decision: let `It` PREFER the `SelfD` mention over a non-self singular. It lands here.
