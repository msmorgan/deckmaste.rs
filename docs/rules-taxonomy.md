...
Open design question: players-as-objects. "Any target" (829 lines;
[CR#115.4]) = creature ∪ player ∪ planeswalker ∪ battle, so the cleanest model
...
| divided among | 82 | division fixed at announce for targets ([CR#601.2d]), at resolution otherwise ([CR#608.2d]) |
...
- **Linked-ability information** ([CR#607] — the full list of linkage
  patterns):
  - cards exiled with this ([CR#607.2a,607.2b,607.2q]; "the exiled card" — 184 lines);
  - objects put onto the battlefield / created by this ([CR#607.2c]);
  - the chosen value — color, type, name, number ([CR#607.2d]; "the chosen
    name" 44 lines, "choose a color" 102, "choose a creature type" 104);
  - noted information ([CR#607.2e]);
  - anchor-word choice ([CR#607.2f], [CR#614.12c]);
  - cost paid as it entered / as it was cast, and whether an optional cost
    was paid — "if kicked" ([CR#607.2g,607.2i,607.2j]; [CR#702.33]);
  - pre-game choices ([CR#607.2n,607.2p]).
...
- **look-back-in-time** semantics for leave/die/lose-control triggers
  ([CR#603.10a..603.10g]) — engine concern, but the Event kind must know which events
  use prior state;
...
[CR#602.1a]), spell costs and their modifiers (additional/alternative,
[CR#118.8,118.9]), and resolution-time payments ("unless you pay…", [CR#118.12]).
...
costs ([CR#118.9], only-one [CR#118.9a]), reductions ("cost {N} less" 561
lines, "more" 128; [CR#118.7]), and per-mode costs (spree/tiered/escalate,
[CR#702.172,702.183,702.120]). Keyword parameters are overwhelmingly Costs — equip,
...
| player | player-affecting ([CR#613.10]) | "You can't lose the game", max hand size |
| rules | game-rule modifying ([CR#613.11]) | cost mods (561+128 lines), "can't" restrictions (1,861 lines), "don't/doesn't" (858), play-permissions ("you may cast … from your graveyard"), requirements ("attacks each combat if able", 251 "if able" lines), "double" (432), "enters tapped" (737) |
...
([CR#611.3]). That's an engine semantic, but the data must distinguish "target
...
A closed set of template shapes the data model should mirror 1:1
([CR#614,615]), since the CR itself enumerates them:
...
Designations attach to *objects*. A zone change creates a new object
([CR#400.7]), so the dominant "until it leaves the battlefield" persistence is
...
|---|---|---|---|---|---|
| monstrous | [CR#701.37b] | marker only | flag | monstrosity N | leaves the battlefield |
| renowned | [CR#702.112b] | marker only | flag | renown N | leaves |
...
| foretold | [CR#702.143d] | may be cast for its foretell cost (which the granting effect may supply) | flag (+ optional attached Cost) | foretell special action ([CR#116.2h]); "becomes foretold" effects | — (object identity: cast / leaves exile) |
| plotted | [CR#702.170c] | castable later without paying its mana cost | flag | plot special action ([CR#116.2k]); "becomes plotted" effects | — (object identity) |

Suspended ([CR#702.62b]) belongs to the vocabulary but not the table: it's a
**derived designation** — defined as a predicate, never granted or stored.
See the derived catalog below.

**Commander** ([CR#903.3]) is the one true exception to object scoping: the CR
...
|---|---|---|
| the monarch | [CR#725.1] | unique-per-game; inherent triggers: end-step draw, transfers on combat damage ([CR#725.2]) |
| the initiative | [CR#726.1] | unique-per-game; inherent venture triggers, transfers on combat damage ([CR#726.2]); re-taking by holder re-triggers but creates no second designation ([CR#726.5]) |
| city's blessing | [CR#702.131c] | non-unique, permanent once gained (ascend) |
| speed | [CR#702.179] | **number 1–4**; "start your engines!" SBA sets 1; inherent once-per-turn increase trigger ([CR#702.179d]); "max speed" = 4 ([CR#702.179e]); no-speed reads as 0 ([CR#702.179f]) |
...
|---|---|---|
| day / night | [CR#731.1] | one-of-two once set, never unset; "day becomes night" = lose first, gain second ([CR#731.1a]); checked in untap step ([CR#502.2], [CR#703.4b]) |
...
|---|---|---|---|
| suspended | object | [CR#702.62b] | in exile ∧ has suspend ∧ has a time counter |
| modified | object | [CR#700.9] | has a counter ∨ equipped ∨ enchanted by controller's Aura |
| poisoned | player | [CR#122.1f] | poison counters ≥ 1 |
| max speed | player | [CR#702.179e] | speed = 4 |
| fully unlocked | object | [CR#709.5i] | has both unlocked designations |
| unpaired | object | [CR#702.95b] | ¬paired |
| descended | player | [CR#700.11] | a permanent card was put into your graveyard this turn (turn memory) |
| enchanted / equipped / fortified | object | [CR#303.4], [CR#301.5] | has an attachment of that kind |
| blocked / unblocked | object | [CR#509.1h] | combat-state relation |
| is your Ring-bearer | object×player | [CR#701.54e] | has the stored Ring-bearer flag ∧ on the battlefield ∧ under your control |
...
stored flags get tested through derived wrappers ([CR#701.54e] adds control and
battlefield checks on top of the Ring-bearer flag). At the fully stateless
end, historic ([CR#700.6] — artifact ∨ legendary ∨ Saga) is the same declaration
...
command (+ ante; + "outside the game", which is not a zone, [CR#400.11]). Each
...
piles, [CR#406.4]), and visibility. Zone-change verbs map pairs of these.
...
(2,733 lines) and -1/-1 (199, annihilation [CR#122.3]), loyalty ([CR#122.1e]),
defense ([CR#122.1g]), shield ([CR#122.1c]), stun ([CR#122.1d]), finality ([CR#122.1h]), lore
([CR#107.15a]), and **keyword counters** ([CR#122.1b] — flying, first strike, …,
...
modes ([CR#700.2d]), other-player choosers ([CR#700.2e]), per-mode costs
(spree/tiered/escalate), pawprint-budget modes ([CR#700.2i]), per-mode targets
([CR#700.2c,115.8]). Plus vote specs ([CR#701.38]), villainous choices ([CR#701.55]), and
pile-splitting ([CR#700.3]). Sagas (523 chapter lines) and Classes/levelers (50
...
- Is `Target` a distinct kind, or `Binding { quantifier, filter }` with
  target as one quantifier? (Retargeting and per-instance identity,
  [CR#115.3,115.7], argue targets stay structurally special.)
