...
Registry data lives in `Check.Facts`; this module interprets its declared columns. The deed
fact types and the three deed-fact tables are the checker's own, so they are declared here
rather than in `Check.FactTypes`, which holds only the generated tables' row types.
-/
...
/-- What the checker knows about each keyword action [CR#701.1], keyed by the label its
declaration names. Every column here is the checker's own — the gate a deontic clause reads,
authored as a bench sentence spells its deed — so the table is written here beside the other two
deed-fact sources rather than generated from the registry. `Proofs.Tables` pins it against the
generated `keywordActionLabels` in both directions: every declared keyword action has a row, and
every row names a declared keyword action. -/
def actFacts : List (KeywordActionLabel × ActFacts) :=
  [ ("Activate",
    ...
    ("Sacrifice",
      { participle := some "sacrificed", dest := some .graveyard, agentRole := playerAgent,
        patientRole := ⟨some ⟨.object, [], permanentTypes⟩, true, some .battlefield⟩,
